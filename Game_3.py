...
#CAMERA_RATE is how fast the pictures in game update
CAMERA_RATE = 100

#EVALUATION_RATE is how often pictures of faces are sent to be assessed for emotion
# EVALUATION_RATE = 3 means every third picture is sent
EVALUATION_RATE = 2
...
def spawn_obstacle(player, x_shift, y_shift):
    block = obstacles_1(30,30)
    block.rect.x = player.rect.x + x_shift
    block.rect.y = player.rect.y + y_shift
    block.player = player
...
def spawn_platform(player, x_shift, y_shift):
    block = Platform(90, 2)
    block.rect.x = player.rect.x + x_shift
    block.rect.y = player.rect.y + y_shift
    block.player = player
...
def proper_RGB(colour_value):
    return min(max(colour_value, 0), 255)
           ...
           #Setto il livello corrente
            ...
            #-------------------------------------------JOYPAD-----------------------------
             ...
             while not done:
                 ...
                 PLAYER_COLOR = (proper_RGB(P_R),proper_RGB(P_G),proper_RGB(P_B))
                 ...
                 picture_time += clock.tick()
                 
                 if picture_time > CAMERA_RATE:
                   ret, frame = camera.read()

                   picture_time = 0
                   pictures_taken += 1

                   if pictures_taken == EVALUATION_RATE:
                       pictures_taken = 0
          ...
          #--------------------------------------------------JOYPAD---------------------------
            ...
            #-----------------------------------------------------TASTIERA-----------------------
                ...
                #--------------------PUNTEGGIO - lvl - ATTEMPT------------------------------------------
                 ...
                 clock.tick(100)
                 #cam_clock.tick(30)
                 screen.blit(frame, (SCREEN_WIDTH - CAMERA_WIDTH - 20,SCREEN_HEIGHT - CAMERA_HEIGHT))
